...
# Вариант 82
# Вычислить сумма ряда, промежуточных членов, заданных шагом
# Шаг, точность, кол-во итераций и значение
# переменной задаются пользоваьелем при вводе

from math import *

# Ввод значения переменной x
x = float(input("Введите значение перменной x: "))
...
if exst is True:
    ...
    for i in range(1, iterations+1):
        t.append(((-1) ** i) * (((2 * x) ** (2 * i ))/factorial(2 * i)))

        # Поиск максимального члена последовательности для построения таблицы
        t_str.append(str(f'{((-1) ** i) * (((2 * x) ** (2 * i ))/factorial(2 * i)):.7g}'))
